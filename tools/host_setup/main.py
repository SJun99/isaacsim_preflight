...
def main():
    ...
    p.add_argument('--preflight', action='store_true', help='Isaac Sim 사전 점검 이미지와 최소 장면만 준비')
    args = p.parse_args()
    info = inspect(ROOT)
    result = assess(info)
    ...
    print('계획: '+('현재 드라이버 유지' if result['driver_ready'] else 'GPU 지원 후보 중 580 계열 드라이버 설치')+
          (' → Docker·Container Toolkit 준비 → Isaac Sim 최소 이미지 → 물리·렌더링 검사'
           if args.preflight else ' → Docker·Container Toolkit 준비 → 이미지 빌드 → CUDA·카메라·기록 검사'))
    if result['errors']:
        raise RuntimeError('\n'.join(result['errors']))
    if args.check:
    ...
    if args.verify:
        ...
        docker = docker_command()
        if args.preflight:
            run([ROOT/'lekiwi', 'preflight', 'verify'], env=project_env(docker))
        else:
            verify(docker)
        return
    ...
    if args.preflight:
        confirm('Isaac Sim 5.1 최소 이미지를 준비하고 큐브 낙하·렌더링을 검사합니다. 최초 다운로드는 오래 걸릴 수 있습니다.')
        run([ROOT/'lekiwi', 'preflight', 'build'], env=project_env(docker))
        run([ROOT/'lekiwi', 'preflight', 'verify'], env=project_env(docker))
    else:
...
if __name__ == '__main__':
    try:
        ...
        if '--preflight' in sys.argv:
            message = message.replace('./lekiwi install', './lekiwi preflight install')
            if isinstance(exc, subprocess.CalledProcessError):
                message = f'사전 점검 명령이 종료 코드 {exc.returncode}로 중단됐습니다. 바로 위의 원인과 로그 경로를 확인하세요.'
